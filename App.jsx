...
function App() {
  ...
  if (results.length) return <main className="results-page"><header><div className="brand">Auto<span>Match</span></div><button className="link-button" onClick={() => { setAnswers({}); setResults([]); setResolvedImages({}); setStep(-1); }}><RotateCcw size={16} /> Recommencer</button></header><section className="results-intro"><div className="eyebrow"><Sparkles size={16} /> Votre sélection personnalisée</div><h1>Voici les voitures faites pour vous.</h1><p>Les résultats sont classés selon vos réponses et vos priorités.</p></section><div className="result-grid">{results.map((car, index) => <article className="car-card" key={car.id}><img src={resolvedImages[car.id] || fallbackImage} alt={`${car.brand} ${car.model}`} onError={(event) => { event.currentTarget.src = fallbackImage; }} /><div className="car-body"><div className="rank">0{index + 1}</div><div className="compatibility">{car.score}% compatible</div><h2>{car.brand} <strong>{car.model}</strong></h2><p className="specs">{car.price.toLocaleString("fr-FR")} € · {car.power} ch · {car.seats} places</p><p className="specs">{car.fuel === "electric" ? car.consumption : `${car.consumption} · ${car.acceleration}s de 0 à 100 km/h`}</p><div className="tags">{car.reasons.slice(0, 3).map((reason) => <span key={reason}><Check size={13} /> {reason}</span>)}</div></div></article>)}</div></main>;
